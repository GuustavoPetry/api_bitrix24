...
const SOURCE_STAGE_ID = "C22:UC_0O4Q1U"; // ID da etapa de busca



const TARGET_STAGE_ID = "C22:UC_H2IVA3"; // ID do stage alvo (alterar somente esse stage)



const fieldCardMigrado = "UF_CRM_1759494223";
const fieldDatahoraMigrate = "UF_CRM_1759494245";
const fieldDataFechamento = "UF_CRM_1759494199";
    ...
    while (hasMore) {
        const response = await fetch(`${BITRIX_DOMAIN}/rest/${WEBHOOK}/crm.deal.list.json`, {
            ...
            body: JSON.stringify({
                ...
                select: ["ID", "STAGE_ID", fieldDataFechamento],
                start: start
            })
        });
...
async function updateClosedate(dealId, closedate) {
    const response = await fetch(`${BITRIX_DOMAIN}/rest/${WEBHOOK}/crm.deal.update.json`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
async function migrateDeals() {
    try {
        ...
        const sourceStageDeals = deals.filter(deal => deal.STAGE_ID === SOURCE_STAGE_ID);
        ...
        for (const deal of sourceStageDeals) {
            const updateStage = await updateDeal(deal.ID, TARGET_STAGE_ID);
            // const closedateResult = await updateClosedate(deal.ID, deal[fieldDataFechamento]);
